        ...
        if refine:
            ...
            while error_class != 11 and max_tries < MAX_TRIES:
                ...
                try:
                    ...
                    refined = ast.literal_eval(str(pred_t))
                    assert isinstance(refined, list) and len(refined) > 0 and all(
                        isinstance(rule, dict) for rule in refined
                    ), f'Re-generation failed for: \n{nlacp}\n'
                    pred_pol = remove_duplicates(refined)
                    error_class = verify(nlacp, pred_pol, ver_model, ver_tokenizer)
